...
# Change your current fastapi import to this:
...
async def run_task_back(complaint_id:int,file_loc:str,description:str,location:str,latitude:float,longiTude:float):
    try:
        ...
        if not ai_result.get("detected"):
            ...
            cursor=conn.cursor()
            # execute
            cursor.execute("UPDATE complaints SET status='rejected' WHERE id=?",
(complaint_id,))
            # commit
            conn.commit()
            # close
        ...
        WHERE id=?
      ''', (logic_result['priority'], logic_result['category'], logic_result['score'],
logic_result['jurisdiction'], complaint_id))
        conn.commit()
        conn.close()
